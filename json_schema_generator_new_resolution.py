...
class json_schema_generator_ui:
    ...
    def __init__(self,root):
        ...
        self.canvas = Canvas(root, width=1300, height=700)
        self.canvas.create_image(0, 0, image = self.bg, anchor = NW)        
        self.my_rectangle = self.round_rectangle(30, 30, 1270, 670,  radius=45, fill="white")
        Label(self.canvas, text="JSON Schema Generator",fg="black", font=("Helvetica", "20", "bold"), bg="white").place(x=450,y=70)
    ...
    def first_frame(self):
        ...
        self.frame1 = LabelFrame(self.canvas, text= "Upload Options", bg="white", font=("Times","10") )
        self.frame1.place(x=50,y=260, width=500, height=300)    
    
        Label(self.frame1, text="File Upload", fg="#383838", font=("Century", "10"), bg="white").place(x=30,y=30)
        self.file_upload = Entry(self.frame1, fg="#383838", font=("Century", "10"), bg="white", textvariable=self.file_name, state=DISABLED)
        self.file_upload.place(x=30,y=70, width=450)
        
        self.button1_bg = ImageTk.PhotoImage(file=final_path+"/button_upload-file-files.png")
        Button(self.frame1,  image=self.button1_bg, borderwidth=0, bg="white",  command=lambda:self.open_file(), state=DISABLED).place(x=120, y=230)  

        Label(self.frame1, text="URL Upload", fg="#383838", font=("Century", "10"), bg="white").place(x=30,y=120)
        self.url_upload = Entry(self.frame1, fg="#383838", font=("Century", "10"), bg="white", textvariable=self.url, state=DISABLED)
        self.url_upload.place(x=30,y=160, width=450)

        self.button2_bg = ImageTk.PhotoImage(file=final_path+"/button_upload-url-urls.png")
        Button(self.frame1,  image=self.button2_bg, borderwidth=0, bg="white",  command=lambda:self.open_file(), state=DISABLED).place(x=290, y=230)  

    def second_frame(self):
        
        self.frame2 = LabelFrame(self.canvas, text= "JSON Schema Viewer",  bg="white", font=("Times","10") )
        self.frame2.place(x=560,y=260, width=690, height=400)  
    
    def third_frame(self):
        
        self.frame3 = LabelFrame(self.canvas, text= "Input Options",  bg="white", font=("Century","10") )
        self.frame3.place(x=50,y=150, width=1200, height=100)    
        Label(self.frame3, text="Input Format",fg="#292626", bg="white",font=("Century", "10")).place(x=30,y=25)
        ...
        self.cmb_box .place(x=150, y=25, width=300) 
        
        Label(self.frame3, text="1. Upload a single file of resource item for any city", fg="#383838", font=("Century", "7"), bg="white").place(x=550,y=7)
        Label(self.frame3, text="2. Upload multiple files of resource item for any city", fg="#383838", font=("Century", "7"), bg="white").place(x=550,y=42)
        Label(self.frame3, text="3. Upload a URL of resource item for any city", fg="#383838", font=("Century", "7"), bg="white").place(x=850,y=7)
        Label(self.frame3, text="4. Upload URLs of resource item for any city", fg="#383838", font=("Century", "7"), bg="white").place(x=850,y=42)

    def fourth_frame(self):
    
        self.frame4 = LabelFrame(self.canvas, text= "Submit Options", bg="white", font=("Times","10") )
        self.frame4.place(x=50,y=570, width=500, height=90)    

        self.submit_button_file = ImageTk.PhotoImage(file=final_path+"/button_generate.png")
        Button(self.frame4, image=self.submit_button_file, borderwidth=0, bg="white", command=self.verify_file, state=DISABLED).place(x=140,y=20)

        self.download_button = ImageTk.PhotoImage(file=final_path+"/button_save.png")
        Button(self.frame4, image=self.download_button, borderwidth=0, bg="white", command=self.saveFile, state=DISABLED).place(x=310,y=20)

    def table_frame(self):

        self.table_frame = Frame(self.frame2, bg="white", relief=RIDGE, bd=4)
        self.table_frame.place(x=20,y=10, width=650, height=350)  
    ...
    def selected(self, event):
        ...
        if self.cmb_box.get() == self.values[0]:
            ...
            Button(self.frame1,  image=self.button1_bg, borderwidth=0, bg="white",  command=lambda:self.open_file(), state=DISABLED).place(x=120, y=230)  
            Button(self.frame1,  image=self.button2_bg, borderwidth=0, bg="white",  command=lambda:self.open_url(), state=DISABLED).place(x=290, y=230)  
            Button(self.frame4, image=self.submit_button_file, borderwidth=0, bg="white", command=self.verify_file, state=DISABLED).place(x=140,y=20)
            Button(self.frame4, image=self.download_button, borderwidth=0, bg="white", command=self.saveFile, state=DISABLED).place(x=310,y=20)
            self.file_upload = Entry(self.frame1, fg="#383838", font=("Century", "10"), bg="white", textvariable=self.file_name, state=DISABLED)
            self.file_upload.place(x=30,y=70, width=450)
            ...
            Button(self.frame1,  image=self.button1_bg, borderwidth=0, bg="white",  command=lambda:self.open_file()).place(x=120, y=230)  
            Button(self.frame1,  image=self.button2_bg, borderwidth=0, bg="white",  command=lambda:self.open_url(), state=DISABLED).place(x=290, y=230)  
            Button(self.frame4, image=self.submit_button_file, borderwidth=0, bg="white", command=self.verify_file).place(x=140,y=20)
            Button(self.frame4, image=self.download_button, borderwidth=0, bg="white", command=self.saveFile).place(x=310,y=20)
            self.file_upload = Entry(self.frame1, fg="#383838", font=("Century", "10"), bg="white", textvariable=self.file_name)
            self.file_upload.place(x=30,y=70, width=450)
            self.url_upload = Entry(self.frame1, fg="#383838", font=("Century", "10"), bg="white", textvariable=self.url, state=DISABLED)
            self.url_upload.place(x=30,y=160, width=450)
            ...
            Button(self.frame1,  image=self.button1_bg, borderwidth=0, bg="white",  command=lambda:self.open_file(), state=DISABLED).place(x=120, y=230)  
            Button(self.frame1,  image=self.button2_bg, borderwidth=0, bg="white",  command=lambda:self.open_url()).place(x=290, y=230)  
            Button(self.frame4, image=self.submit_button_file, borderwidth=0, bg="white", command=self.verify_file).place(x=140,y=20)
            Button(self.frame4, image=self.download_button, borderwidth=0, bg="white", command=self.saveFile).place(x=310,y=20)
            self.file_upload = Entry(self.frame1, fg="#383838", font=("Century", "10"), bg="white", textvariable=self.file_name, state=DISABLED)
            self.file_upload.place(x=30,y=70, width=450)
            self.url_upload = Entry(self.frame1, fg="#383838", font=("Century", "10"), bg="white", textvariable=self.url)
            self.url_upload.place(x=30,y=160, width=450)
